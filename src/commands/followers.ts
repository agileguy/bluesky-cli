...
export function createFollowersCommand(config: ConfigManager): Command {
  return new Command('followers')
    .description('List followers for a user (defaults to current user)')
    .argument('[handle]', 'User handle (e.g., @alice.bsky.social or alice.bsky.social)')
    .option(
      '-l, --limit <number>',
      'Maximum number of followers to retrieve (default: 50, max: 100)',
      '50'
    )
    ...
    .action(async (handleArg: string | undefined, options) => {
      try {
        ...
        if (options.json) {
          ...
          console.log(chalk.cyan(`\nFollowers of @${targetHandle}:`));
          console.log(
            chalk.gray(`Showing ${followers.length} follower${followers.length !== 1 ? 's' : ''}\n`)
          );

          if (followers.length === 0) {
            console.log(chalk.gray('  No followers found'));
          } else {
            followers.forEach((follower, index) => {
              console.log(formatter.formatProfile(follower));
